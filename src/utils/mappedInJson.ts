interface ApIndexForecast {
    observed?: string;
    estimated?: string;
    predicted?: string;
}

interface GeomagneticProbabilities {
    [key: string]: string; // ключи - типы активности, значения - вероятности
}

interface KpIndexForecast {
    timeSlot: string;
    values: string[];
}

interface Result {
    apIndexForecast: ApIndexForecast;
    geomagneticProbabilities: GeomagneticProbabilities;
    kpIndexForecast: KpIndexForecast[];
}

export const extractData = (text: string): Result => {
